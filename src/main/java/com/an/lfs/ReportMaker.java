...
public class ReportMaker extends AbstractReportMaker {
     ...
     * @throws Exception
     */
    public void make(Country country, Map<Integer, List<MatchInfo>> yearMatchMap, RateLoader rateLoader,
            MatchRuleMgr matchRuleMgr) throws Exception {
        ...
        for (int i = 0; i < totalYear; i++) {
            if (i > 0) {
                matchRowList.add(getBlankRow(matchReportHeadNames.length));
            }
    ...
    private WritableCellFormat getEndRateFont(float rate, float rateEnd) throws WriteException {
    ...
    private String getTopLastStr(boolean isHostTopN, boolean isGuestTopN, boolean isHostLastN, boolean isGuestLastN,
            String host, String guest, Map<String, Integer> teamRank) {
        String ret = "";
        if (isHostTopN) {
            ret = "TN";
        } else if (isHostLastN) {
    ...
    private Integer getTeamRank(Map<String, Integer> teamRank, String team) {
    ...
    private List<Cell> getMatchReportHead() {
    ...
    public void makeLeague(Country country, Map<Integer, List<MatchInfo>> yearMatchMap, RateLoader rateLoader,
            MatchRuleMgr matchRuleMgr) throws Exception {
        if (yearMatchMap.isEmpty()) {
            return;
        }
    ...
    private List<List<Cell>> getRuleRows(MatchRuleMgr matchRuleMgr) {
    ...
    private Comparator<List<Cell>> leagueRowComparator = new Comparator<List<Cell>>() {
        @Override
    ...
    private void addCompanyRate(MatchRate matchRate, Result scoreRet, List<Cell> row, MatchRuleMgr matchRuleMgr)
    ...
    private String getBetRetValue(Result scoreRet, float win, float draw, float lose) {
    ...
    private WritableCellFormat getRateValueFormat(String betRetVal) throws WriteException {
    ...
    private List<Cell> getLeagueMatchReportHead() {
        List<Cell> head = new ArrayList<>();
